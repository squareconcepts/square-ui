<div class="flex flex-col mb-4 {{$attributes->has('disabled') ? 'opacity-60' : ''}}" x-data="{value: $wire.entangle('{{ $attributes['wire:model'] }}').live}">
    ...
    <div class="relative rounded-md">
        ...
        <input {{ $attributes->merge(['class' => $getClasses($errors->has($name)) ]) }}  type="{{$type}}" name="{{ $name }}" {{ $attributes->except('wire:model') }} placeholder="{{$placeholder ?? $label}}"  @if(!$show1Password) data-1p-ignore @endif x-model="value"/>
    </div>
