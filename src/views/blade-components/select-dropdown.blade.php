<div class="relative {{$classes}}" x-data="{
        ...
        async setInitialKey () {
            ...
            if ((this.initialValue != null && this.selectedKey === '') || this.multiple) {
                if (this.multiple) {
                    items = results.filter((result) => this.initialValue?.includes(result[this.optionValue]) ?? false);
...
>
    <label class="block text-sm font-medium text-gray-700 dark:text-gray-400" x-html="label">
    </label>
    <div class="input-style placeholder-secondary-400 dark:bg-secondary-800 dark:text-secondary-400 dark:placeholder-secondary-500 border border-secondary-300 focus:ring-primary-500 focus:border-primary-500 dark:border-secondary-600 form-input block w-full sm:text-sm rounded-md transition ease-in-out duration-100 focus:outline-none shadow-sm cursor-pointer !flex items-center" x-on:click="open = !open">
        <div x-show="!multiple">
            <p x-text="selectedKey"
               ...
               x-cloak></p>
            <p x-html="placeholder" x-show="!selectedValue" x-cloak class="text-sm text-gray-400"></p>
        </div>
        <div x-show="multiple" class="flex flex-wrap gap-1" x-cloak>
            ...
            <p x-html="placeholder" x-show="!selectedValue || selectedValue.length == 0" x-cloak class="text-sm text-gray-400"></p>
        </div>
        <div class="ml-auto flex gap-2">
            <svg class="w-5 h-5 text-secondary-400" xmlns="http://www.w3.org/2000/svg" fill="none"
                 viewBox="0 0 24 24" stroke="currentColor" @click="reset($event)">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 8L8 16M8 8L16 16"></path>
            </svg>
            <svg class="w-5 h-5 text-secondary-400" xmlns="http://www.w3.org/2000/svg" fill="none"
        ...
        <div class="max-h-48 overflow-auto" x-show="results.length > 0" x-cloak>
            <template x-for="(result, index, collection) in results" :key="index">
                <div
                    ...
                    :class="typeof result !== 'undefined' && selectedValue != null && (multiple ? (selectedValue.includes(result[`${optionValue}`])) : (selectedValue == result[optionValue])) ? 'font-bold hover:bg-red' :'hover:bg-sc-blue'"
                ...
                >
                    <div x-show="multiple && typeof selectedValue == 'object'">
                        <div x-show="typeof selectedValue == 'object' && typeof result !== 'undefined' && selectedValue?.includes(result[optionValue])"
                           class="flex items-center justify-between">
                            <div>
                                <span x-text="typeof result !== 'undefined' ? result[optionLabel] : ''"></span>
                                <span class="text-xs opacity-70 block" x-show="optionDescription !== null && optionDescription.length > 0" x-text="typeof result !== 'undefined' ? result[optionDescription] : ''"></span>
                            </div>
                            <i class="fa fa-check"></i>
                        </div>
                        <div x-show="typeof result !== 'undefined' && typeof selectedValue == 'object' && !selectedValue?.includes(result[optionValue])">
                            <span x-text="typeof result !== 'undefined' ? result[optionLabel] : ''"></span>
                            <span class="text-xs opacity-70 block" x-show="optionDescription !== null && optionDescription.length > 0" x-text="typeof result !== 'undefined' ? result[optionDescription] : ''"></span>
                        </div>
                    </div>
                    ...
                    <div x-show="!multiple">
                        <div x-show="typeof result !== 'undefined' && selectedValue == result[optionValue]"
                           class="flex items-center justify-between">
                            <div>
                                <span x-text="typeof result !== 'undefined' ? result[optionLabel] : ''"></span>
                                <span class="text-xs opacity-70 block" x-show="optionDescription !== null && optionDescription.length > 0" x-text="typeof result !== 'undefined' ? result[optionDescription] : ''"></span>
