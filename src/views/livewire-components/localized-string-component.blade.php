<div class="flex flex-col gap-2 relative" x-data="{showDropdown: false}">
    <x-square-ui::dropdown>
        <x-slot:triggerSlot>
            <x-input wire:model="value.{{ config('app.locale') }}" class="py-2" style="padding-left: 40px" wire:keyup="emitValue">
                <x-slot name="prepend">
        ...
        <div class="flex flex-col gap-2">
            @foreach(config('square-ui.languages') as $lang => $flag)
                @if($lang != config('app.locale'))
                    <x-input wire:model="value.{{ $lang }}" class="py-2" style="padding-left: 40px;" wire:keyup="emitValue">
                        <x-slot name="prepend">
